...
def all_blog_posts(request):
    ...
    using pagination to show 3 blogs per page"""
    ...
    try:
        ...
        blog_page = paginator.page(default_page)
    except EmptyPage:
...
def add_blog_post(request):
    ...
    if not request.user.is_superuser:
        messages.error(
            request,
            "Sorry, only authenticated authors are allowed to add to the blog!"
            )
...
def delete_blog(request, blog_id):
    ...
    if not request.user.is_superuser:
        messages.error(
            request,
            'Sorry, only store owners and authors are allowed to do that!')
...
def edit_blog(request, blog_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                'Failed to edit product, please ensure form is valid')
    else:
        form = BlogPostForm(instance=blog)
        messages.info(
            request,
            f'You are editing {blog.title} originally written by {blog.author}'
        )
